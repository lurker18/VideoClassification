...
import torch
from core.mvit.utils import Logger
from torch.utils.data import DataLoader
from torch import optim, nn
from core.mvit import get_model, get_dataset, training
from core.mvit.training import train_epoch
from core.mvit.validation import val_epoch
from core.mvit.test import test
from core.mvit.get_model import MODELS
...
def run(args, logger):
    ...
    model, parameters = get_model.fetch_model(logger, args.model_type, train_dataset=args.train_dataset,
                                                pretrain_dataset=args.pretrain_dataset,
                                                pretrained_pth=args.pretrain_pth)
    model.cuda()

    logger.info(f"Completed building {args.model_type}")
    test_dataset = get_dataset.ActionRecognitionUniformFrames(args, 'test')
    ...
    if args.do_train:
        train_dataset = get_dataset.ActionRecognitionUniformFrames(args, 'train')
        val_dataset = get_dataset.ActionRecognitionUniformFrames(args, 'val')
